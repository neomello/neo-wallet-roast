...
export default function HomeClient() {
  ...
  const [roastData, setRoastData] = useState<{ roast: string; score: number; labels?: string[]; error?: string } | null>(null);
  const [error, setError] = useState<string | null>(null);

  const handleRoast = async (e?: React.FormEvent, isDemo: boolean = false) => {
    e?.preventDefault();
    ...
    try {
      const response = await fetch('/api/roast', {
        ...
        body: JSON.stringify({ address: address || '0xd8da6bf26964af9d7eed9e03e53415d37aa96045', isDemo }),
      });
  ...
  return (
    <main className="max-w-2xl mx-auto px-6 py-12 flex flex-col items-center min-h-[100dvh]">
      ...
      >
        <form onSubmit={(e) => handleRoast(e)} className="flex flex-col gap-6">
            <div className="relative group">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-fire-400 group-focus-within:text-fire-500 transition-fire w-6 h-6" />
                <input 
                    type="text" 
                    placeholder="Enter ETH Address..."
                    className="w-full bg-black/50 border-2 border-fire-900 focus:border-fire-500 rounded-xl py-4 pl-14 pr-4 transition-fire outline-none text-white text-lg font-mono placeholder:text-zinc-600"
                    value={address}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                ...
                >
                    {isLoading ? <RefreshCw className="w-6 h-6 animate-spin"/> : " incinerate wallet "}
                </button>
                <button 
                   disabled={isLoading}
                   type="button"
                   onClick={() => handleRoast(undefined, true)}
                   className="flex-none bg-zinc-900 hover:bg-zinc-800 text-zinc-400 font-bold py-4 px-8 rounded-xl transition-fire uppercase tracking-widest border border-zinc-800"
                >
                    demo mode
                </button>
            </div>
          ...
          >
            <div className="card-fire p-8 overflow-hidden relative">
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                    {roastData.labels?.map((label: string) => (
                        <span key={label} className="bg-fire-500/20 border border-fire-500/40 text-fire-300 px-3 py-1 rounded-full text-sm font-bold uppercase tracking-wider">
                            #{label}
                        </span>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-fire-900/50 pt-8 mt-4">
                    <div className="text-center">
                     ...
                     <div className="text-center">
                        <div className="text-4xl font-black text-white">{roastData.score > 80 ? "INSANE" : roastData.score > 50 ? "DECENT" : "BORING"}</div>
